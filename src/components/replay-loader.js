...
AFRAME.registerComponent("replay-loader", {
  ...
  update: function () {
    ...
    if (this.data.link.length) {
      ...
      this.userIds = this.data.playerID.length
        ? [this.data.playerID]
        : this.data.players.split(",");
      document.addEventListener("songFetched", (e) => {
        captureThis.downloadReplay(
          this.data.hash ? this.data.hash : e.detail.hash
        );
      });
    }
  ...
  downloadReplay: function (hash) {
    ...
    this.userIds.forEach((playerID, index) => {
      fetch(
        `https://api.beatleader.xyz/score/${playerID}/${hash}/${this.data.difficulty}/${this.data.mode}`
      ).then(async (response) => {
            ...
            if (replay && replay.frames) {
              if (replay.frames.length == 0) {
                this.el.sceneEl.emit(
                  ...
                  {
                    error:
                      "Replay broken, redownload and reinstall mod, please",
                  },
                  null
                );
              } else {
                ...
                const jd =
                  replay.info.jumpDistance > 5
                    ? replay.info.jumpDistance
                    : undefined;
                this.el.sceneEl.emit(
                  "replayfetched",
                  {
                ...
                this.el.sceneEl.emit(
                  "colorChanged",
                  { index, color: replay.color, playerID },
                  null
                );
                if (this.challenge) {
  ...
  downloadSSReplay: function (hash, playerID, index) {
    fetch(
      `/cors/score-saber/api/leaderboard/by-hash/${hash}/info?difficulty=${difficultyFromName(
      ...
      res.json().then((leaderbord) => {
        ...
        //   const index = i;
        fetch(
          `${DECODER_LINK}/?playerID=${playerID}&songID=${leaderbord.id}`
        ).then((res) => {
    ...
    fetch(`/cors/score-saber/api/player/${playerID}/full`, {
      ...
      res.json().then((data) => {
        const user = {
          name: data.name,
          avatar: data.profilePicture.replace(
            "https://cdn.scoresaber.com/",
            "/cors/score-saber-cdn/"
          ),
  ...
  fetchSSFile: function (file, itsLink) {
    ...
    let playerId = (itsLink ? file : file.name)
      .split(/\.|-|\//)
      .find((el) => (el.length == 16 || el.length == 17) && parseInt(el, 10));
    if (playerId) {
    ...
    fetch(`/cors/score-saber/api/player/${playerID}/full`, {
      ...
      res.json().then((data) => {
        ...
        this.el.sceneEl.emit(
          ...
          {
            name: this.user.name,
            avatar: this.user.profilePicture.replace(
              "https://cdn.scoresaber.com/",
              "/cors/score-saber-cdn/"
            ),
  ...
  processScores: function (replay) {
    const map =
      this.challenge.beatmaps[this.challenge.mode][this.challenge.difficulty];
    ...
    for (var i = 0; i < replay.notes.length; i++) {
      ...
      if (
        note.id == -1 ||
        (note.id > 0 && note.id < 100000 && note.id % 10 == 9)
      ) {
    ...
    const processGroup = () => {
      for (var j = 0; j < group.length; j++) {
        ...
        for (var m = 0; m < group.length; m++) {
          ...
          const lineIndex = leftHanded
            ? 3 - mapnote._lineIndex
            : mapnote._lineIndex;
          const colorType = leftHanded ? 1 - mapnote._type : mapnote._type;
          const cutDirection = leftHanded
            ? mirrorDirection(mapnote._cutDirection)
            : mapnote._cutDirection;
          const lineLayer = mapnote._lineLayer;
          const scoringType = mapnote._scoringType
            ? mapnote._scoringType + 2
            : 3;
          const id =
            lineIndex * 1000 + lineLayer * 100 + colorType * 10 + cutDirection;

          if (
            replaynote.index == undefined &&
            (replaynote.id == id || replaynote.id == id + scoringType * 10000)
          ) {
    ...
    this.el.sceneEl.emit(
      ...
      {
        notes: allStructs,
        allProcessed: this.processedScores == this.userIds.length,
      },
      null
    );
    if (this.processedScores == this.userIds.length) {
      this.noteCountForBattle = Math.round(
        this.allStructs.length / (this.replays.length - 1)
      );
    }
  },
