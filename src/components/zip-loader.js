...
AFRAME.registerComponent("zip-loader", {
  ...
  processFiles: function (loader, isDragDrop) {
    ...
    if (
      ...
      event.mappingExtensions =
        event.info._customData._editorSettings.modSettings.mappingExtensions;
    }
    ...
    beatmapSets.forEach((set) => {
      ...
      diffBeatmaps.forEach((diff) => {
        ...
        if (
          ...
          (diff._customData._requirements.indexOf("Mapping Extensions") !==
            -1 ||
    ...
    if (!event.difficulty) {
      event.difficulty =
        ...
        event.difficulties[this.data.mode][0]._difficulty;
    }
  ...
  fetchData: function (id, byHash) {
    document.cookie =
      "aprilFools=1; expires=Sat, 03 Apr 2022 00:00:00 UTC; path=/";
    return fetch(
      `/cors/beat-saver2/api/maps/${byHash ? "hash" : "id"}/${id}`
    ).then((res) => {
      res.json().then((data) => {
        if (data.versions) {
          ...
          if (desiredHash.toLowerCase() == currentVersion.hash.toLowerCase()) {
            ...
            let urls = ["r2cdn", "cdn"].map(
              (prefix) =>
                "https://" +
                prefix +
                ".beatsaver.com/" +
                desiredHash.toLowerCase() +
                ".zip"
            );
  ...
  fetchZip: function (zipUrl, fallbackUrl) {
    ...
    if (
      ...
      (this.data.id && this.fetchedZip & (this.fetchedZip === this.data.id))
    ) {
      return;
    }
...
function generateMode(event, difficulty, mode) {
  if (mode.includes("Standard")) {
    ...
    event.beatSpeeds[mode][difficulty] =
      event.beatSpeeds["Standard"][difficulty];
    event.beatOffsets[mode][difficulty] =
      event.beatOffsets["Standard"][difficulty];
    ...
    switch (mode) {
      ...
      case "InvertedStandard":
        Mirror_Inverse(
          event.beatmaps[mode][difficulty],
          4,
          false,
          false,
          false
