...
AFRAME.registerComponent('beat-generator', {
  ...
  doGenerateBeat: (function () {
    ...
    return function (beatEl, note, type, color) {
      ...
      beatObj.anticipationPosition = -this.beatAnticipationTime * this.beatSpeed - this.settings.settings.magicConstant;
      beatObj.color = color;
  ...
  generateWall: (function () {
    ...
    return function (wall) {
      ...
      wallObj.anticipationPosition =
        -this.beatAnticipationTime * this.beatSpeed - this.settings.settings.magicConstant;
    ...
    if (!itsDefault) {
      for (let i = 0; i < this.beatContainer.children.length; i++) {
        ...
        if (child.components.beat) {
          ...
          child.components.beat.data.anticipationPosition = -this.beatAnticipationTime * this.beatSpeed - this.settings.settings.magicConstant;
        }
        if (child.components.wall) {
          child.components.wall.data.anticipationTime = this.beatAnticipationTime;
          child.components.wall.data.anticipationPosition = -this.beatAnticipationTime * this.beatSpeed - this.settings.settings.magicConstant;
        }
      }
    }
  },
